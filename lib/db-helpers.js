...
export const buildSearchConditions = (searchParams) => {
  ...
  if (search) {
    ...
    switch (normalized.type) {
      case 'email':
        ...
        break
      case 'keyid':
        conditions["key.keyid"] = { like: `%${normalized.value}` }
        break
      case 'fingerprint':
        conditions["key.fingerprint"] = normalized.value
        break
      case 'name':
        joins.push('userIds')
  ...
  // Status filters
  if (!include_revoked) {
    // Specify that we are checking the status of the primary key
    conditions['key.revoked'] = false
  }
  if (!include_expired) {
    // Specify that we are checking the status of the primary key
    conditions['key.expired'] = false
  }
